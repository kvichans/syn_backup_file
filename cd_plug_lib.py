...
#           t       = bs_cnt['t'] + top_plus_for_os(tp, REDUCTS.get(bs_tp, bs_tp))
    ...
    pass;                      #log('title, w, h={}',(title, w, h))
    pass;                      #log('enum(ctrls_l)={}',pformat(list(enumerate(ctrls_l)), width=120))
    pass;                      #open(r'c:\Programs\SynWrite\Py\syn_backup_file\ctrls.txt', 'w').write('\n'.join(ctrls_l).replace('\r',''))
    ...
    pass;                      #log('btn_i, cnts[btn_i]={}',(btn_i, cnts[btn_i])) if 'cid' not in cnts[btn_i] else 0
